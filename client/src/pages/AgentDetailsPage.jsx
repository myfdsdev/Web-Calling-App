...
function Chips({ items }) {
  if (!items?.length) return <span className="italic text-ink-soft">Not set</span>;
  return (
        ...
        <motion.div variants={pageVariants} initial="hidden" animate="show" className="space-y-5">
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <DetailCard icon={Briefcase} title="Agent Role">
              <p className="font-medium text-ink">{agent.purpose || 'General assistant'}</p>
              <p className="mt-2 text-[11px] font-semibold uppercase tracking-wide text-ink-soft">Services</p>
              <div className="mt-1">
                <Chips items={agent.services} />
              </div>
            </DetailCard>

            <DetailCard icon={Building2} title="Business">
              <p className="font-medium text-ink">{agent.businessName || 'Not set'}</p>
              <p className="mt-1">{agent.businessType || ''}</p>
            </DetailCard>

            <DetailCard icon={Mic} title="Voice">
              <p className="font-medium text-ink">{agent.voiceName || 'Default'}</p>
              <p className="mt-1 capitalize">{agent.voiceProvider}</p>
            </DetailCard>

            <DetailCard icon={Languages} title="Languages">
              <Chips items={agent.languages} />
              <p className="mt-3 text-[11px] font-semibold uppercase tracking-wide text-ink-soft">Tone</p>
              <div className="mt-1">
                <Chips items={agent.tone} />
              </div>
            </DetailCard>
          </div>
